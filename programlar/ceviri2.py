    ...
    def check_translation(self, msgid: str, msgstr: str, 
        ...
        if not msgstr.strip():
            return GeminiResult(
                ...
                issues=["Çeviri boş"],
                suggestion=None,
                comment="Çeviri yapılmamış",
                status=QualityStatus.POOR
            )
        
        prompt = f"""Sen profesyonel bir çeviri kalite kontrol uzmanısın.
...
Lütfen aşağıdaki kriterlere göre değerlendir:

1. ANLAM BÜTÜNLÜĞÜ (0-100): Çeviri orijinal anlamı koruyor mu?
2. DİLBİLGİSİ (0-100): Hedef dilde dilbilgisi doğru mu?
3. DOĞALLIK (0-100): Çeviri doğal ve akıcı mı, yoksa robotik mi?
4. TEKNİK DOĞRULUK (0-100): Teknik terimler, placeholder'lar (%s, {{{{x}}}}, %d vb.) korunmuş mu?
5. FORMAT (0-100): HTML tag'ler, noktalama işaretleri doğru mu?

Ayrıca:
- Sorunları listele (varsa)
- Daha iyi bir alternatif öner (sadece gerçekten gerekiyorsa)
- Genel bir yorum yap
...
{{
    ...
    "suggestion": "önerilen çeviri veya null",
    "comment": "genel yorum"
}}"""
    ...
    def run(self):
        try:
            ...
            for i, item in enumerate(items_to_check):
                ...
                self.progress.emit(
                    progress_pct,
                    f"🤖 Kontrol ediliyor: '{item.msgid[:50]}...'"
                )
...
class MainWindow(QMainWindow):
    ...
    def create_gemini_tab(self):
        ...
        btn_gemini_browse = QPushButton("📂 Seç")
        ...
        self.gemini_api_status = QLabel("📊 Google Gemini API: ✅ Yapılandırılmış")
...
"""
    ...
    def gemini_item_checked(self, index, item_result):
        """Her kontrol edilen item için kart oluştur"""
        ...
        # Sorunlar
        if result.issues:
            issues_text = "<b>⚠️ Sorunlar:</b><br>" + "<br>".join(f"• {issue}" for issue in result.issues)
        ...
        if result.suggestion and result.suggestion != item.msgstr:
            suggestion_label = QLabel(f"<b>💡 Öneri:</b> {result.suggestion}")
